import json
import inspect
from pydantic import create_model, Field, BeforeValidator
from typing import Optional, Callable, Type, List, Any, Union, Dict, Annotated

from .agent import Agent


def _coerce_to_str(value):
    """Coerce LLM-returned values to string for dynamic ActionInput fields.

    LLMs (notably gemini-2.5-flash-lite) sometimes echo dict/list structures
    where the dynamic schema declared str. Serialize those losslessly so
    pydantic validation does not fail downstream.
    """
...
_CoercedStr = Annotated[str, BeforeValidator(_coerce_to_str)]
_CoercedOptStr = Annotated[Optional[str], BeforeValidator(_coerce_to_str)]
...
class CustomizeAgent(Agent):
    ...
    def create_customize_action(
        ...
        # Use coerced str types so that LLM responses returning dict/list values
        # are auto-serialized to JSON strings instead of failing pydantic validation.
        action_input_fields = {}
        for field in inputs:
            required = field.get("required", True)
            if required:
                action_input_fields[field["name"]] = (_CoercedStr, Field(description=field["description"]))
            else:
                action_input_fields[field["name"]] = (_CoercedOptStr, Field(default=None, description=field["description"]))
