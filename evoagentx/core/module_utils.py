...
def parse_json_from_text(text: str) -> List[str]:
    ...
    # ALWAYS also scan the full text. The fence regex above is non-greedy,
    # so when triple-backticks appear inside a JSON string value (e.g. an
    # LLM prompt containing "do not use ```python``` fences"), the fenced
    # block captures a truncated fragment whose accidental inner braces
    # defeat the old `if not matches:` fallback and hide the real outer
    # envelope. Letting the full-text pass run unconditionally adds the
    # complete object as a candidate; required-keys filtering and length
    # sort downstream pick the right one.
    full_found = pattern.findall(text)
    for m in full_found:
        if m not in matches:
            matches.append(m)
...
def _double_invalid_backslashes(s: str) -> str:
    """Promote a backslash that is not part of a valid JSON escape into ``\\\\``.

    LLM outputs frequently embed LaTeX fragments (``\\(``, ``\\sqrt``, ``\\cos``)
    inside JSON string values. JSON only permits ``\\\"  \\\\  \\/  \\b  \\f
    \\n  \\r  \\t  \\uXXXX``; any other ``\\X`` makes both ``json.loads`` and
    ``yaml.safe_load`` reject the document. This function rewrites only the
    invalid occurrences, leaving every legal escape intact -- including
    pre-escaped backslashes (``\\\\sqrt``), since the regex consumes the pair
    as a unit.
    """
    def _fix(match):
        nxt = match.group(1)
        ...
        return '\\\\' + nxt
    return regex.sub(r'\\(.)', _fix, s, flags=regex.DOTALL)


def repair_and_load_json(json_str: str):
    """Parse an LLM-emitted JSON snippet, applying conservative repairs on
    failure.

    Repair strategies are tried in order from least to most invasive so a
    well-formed payload is never altered. The function returns the parsed
    object on success; on exhaustion it raises ``ValueError`` with the last
    underlying parser error.
    """
    if not isinstance(json_str, str):
        raise ValueError("repair_and_load_json expects a string input")

    candidates = [json_str]

    repaired = _double_invalid_backslashes(json_str)
    if repaired != json_str:
        candidates.append(repaired)

    if r'\"' in json_str:
        unescaped = json_str.replace(r'\"', '"')
        candidates.append(unescaped)
        unescaped_repaired = _double_invalid_backslashes(unescaped)
        if unescaped_repaired != unescaped:
            candidates.append(unescaped_repaired)

    # Some models (e.g. gemini) wrap the entire JSON body in an extra string
    # layer, e.g. '"{\"k\": \"v\"}"'. Strip the outer quotes and let
    # json.loads handle the inner unescape, then push the inner payload as a
    # candidate.
    ...
    if len(stripped) >= 2 and stripped[0] == '"' and stripped[-1] == '"':
        try:
            ...
            if isinstance(inner, str):
                ...
                inner_repaired = _double_invalid_backslashes(inner)
                if inner_repaired != inner:
                    candidates.append(inner_repaired)
        except Exception:
            pass

    # Last resort: when LLM emits LaTeX commands like \boxed{}, the leading
    # backslash can be a *valid* JSON escape (\b == backspace) so the
    # _double_invalid_backslashes repair leaves it untouched but the parsed
    # value loses the LaTeX backslash. As a final candidate, double *every*
    # backslash that precedes a letter so LaTeX survives -- this preserves
    # the literal backslash at the cost of treating \n / \t as literal text.
    aggressive = regex.sub(r'\\([a-zA-Z])', r'\\\\\1', json_str)
    if aggressive != json_str and aggressive not in candidates:
    ...
    raise ValueError(
        f"The following generated text is not a valid JSON string!\n{json_str}"
    ) from last_err
...
def parse_json_from_llm_output(text: str) -> dict:
    ...
    if json_list:
        ...
        try:
            data = repair_and_load_json(json_text)
        except ValueError:
            raise
        except Exception:
