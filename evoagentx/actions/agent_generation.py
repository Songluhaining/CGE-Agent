...
class GeneratedAgent(BaseModule):
    ...
    def validate_prompt(cls, agent: 'GeneratedAgent'):
        ...
        def fix_output_names(match):
            ...
            # Allow leading whitespace before `##` so prompts that nest the
            # Output Format block under an indented list (common in Gemini
            # outputs) still surface the headings.
            matches = re.findall(r"^\s*##\s+([^\n#]+?)\s*$", output_format, flags=re.MULTILINE)
            generated_outputs = [m.strip() for m in matches if m.strip().lower() != "thought"]
            outputs_names_lower = {n.lower() for n in outputs_names}
            generated_outputs_lower = {g.lower() for g in generated_outputs}
            # Strict path: every declared output appears as its own ## heading.
            if (outputs_names_lower == generated_outputs_lower
                    and len(generated_outputs) == len(outputs_names)):
                return "### Output Format" + output_format
            # Tolerant fallback: accept the prompt as long as every declared
            # output name appears somewhere in the Output Format body
            # (case-insensitive). Only raise if the model has zero anchor.
            body_lower = output_format.lower()
            missing = [n for n in outputs_names if n.lower() not in body_lower]
            if missing:
                raise ValueError(
                    "The Output Format block does not mention required output "
                    f"name(s) {missing}; declared outputs are {outputs_names}; "
                    f"detected `##` headings: {generated_outputs}."
                )
            for generated_output in generated_outputs:
                if generated_output not in outputs_names:
                    ...
                    if most_similar_output_name and most_similar_output_name != generated_output:
                        output_format = output_format.replace(generated_output, most_similar_output_name)
                        logger.warning(f"Renamed prompt heading '{generated_output}' -> '{most_similar_output_name}' to match agent outputs {outputs_names}.")
            return "### Output Format" + output_format
