...
class CustomizeAction(Action):
        ...
        **kwargs
        ...
        for param in prompt_params_names:
            ...
            if isinstance(value, str):
                ...
                raise TypeError(
                    f"The input type {type(value)} is invalid! "
                    "Valid types: [str, dict, list, int, float, bool, None]."
                )
    ...
    def _try_parse_with_fallbacks(self, llm_output_content: str):
        """Try parsing with self.parse_mode, then 'title', then 'str'.

        Returns the parsed output on first success, or raises the last exception.
        Useful when an LLM (e.g. gemini) does not strictly follow the requested
        output format.
        """
        primary_mode = self.parse_mode
        modes_to_try = [primary_mode]
        for fb in ("title", "str"):
            if fb not in modes_to_try:
        ...
        for mode in modes_to_try:
            try:
                parsed_output = self.outputs_format.parse(
                    ...
                    parse_mode=mode,
                    parse_func=getattr(self, 'parse_func', None) if mode == primary_mode else None,
                    title_format=getattr(self, 'title_format', "## {title}")
                )
                if mode != primary_mode:
                    logger.info(f"Parsed output using fallback parse_mode='{mode}' (primary='{primary_mode}' failed).")
    ...
    def _extract_output(self, llm_output: Any, llm: BaseLLM = None, **kwargs):
        ...
        # Try the configured parse_mode, then cascade to 'title' and 'str'
        # before falling back to a second LLM call. Robust against models
        # (e.g. gemini) that don't always honour the requested format.
        try:
            return self._try_parse_with_fallbacks(llm_output_content)
        except Exception as e:
            logger.info(f"Failed to parse with cascaded parse modes: {e}")
    ...
    async def _async_extract_output(self, llm_output: Any, llm: BaseLLM = None, **kwargs):
        ...
        # Cascade parse modes before invoking LLM-based extraction.
        # See sync variant above for rationale.
        try:
            return self._try_parse_with_fallbacks(llm_output_content)
        except Exception as e:
            logger.info(f"Failed to parse with cascaded parse modes: {e}")
