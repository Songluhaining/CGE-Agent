...
class WorkFlow(BaseModule):
    ...
    async def async_execute(self, inputs: dict = {}, **kwargs) -> str:
        ...
        if failed:
            ...
            if bool(kwargs.get("raise_on_error", False)):
                raise RuntimeError(error_message.content) from last_exception
            return "Workflow Execution Failed"
        
        # Try direct extraction from execution_data first (avoids an extra LLM call
        # that can mangle the answer format, e.g. drop oxed{} in MATH answers).
        direct_output = self._try_direct_output_extraction()
        if direct_output is not None:
            logger.info("Extracted workflow output directly from execution_data.")
            return direct_output

        logger.info("Extracting WorkFlow Output via LLM ...")
    ...
    async def _async_execute_task_by_agents(self, task: WorkFlowNode, next_action: NextAction):
        ...
        while next_action:
            ...
            if self.is_task_completed(task=task):
                break
            # Fallback: force-fill missing output keys from agent raw response
            self._force_fill_missing_outputs(task=task, message=message)
            if self.is_task_completed(task=task):
                break
            next_action: NextAction = await self.workflow_manager.schedule_next_action(
                goal=self.graph.goal,
    ...
    def _force_fill_missing_outputs(self, task, message):
        """Force-fill missing task outputs from agent raw response to prevent infinite loops."""
        task_outputs = [output.name for output in task.outputs]
        current_data = self.environment.get_all_execution_data()
        missing = [o for o in task_outputs if o not in current_data]
        if not missing:
            return
        ...
        if hasattr(message, "content"):
            ...
            if hasattr(content, "content"):
                raw_text = str(content.content) or ""
            elif isinstance(content, str):
                raw_text = content
            elif isinstance(content, dict):
                for m in missing:
                    if m in content:
                        self.environment.execution_data[m] = content[m]
                return
            else:
                raw_text = str(content)
        if not raw_text:
            return
        # Try JSON extraction from raw text. Use the project's brace-matching
        # extractor + repair pipeline so nested objects (common when LLMs echo
        # the example payload) are recovered instead of truncated.
        try:
            import json as _json
            from ..core.module_utils import parse_json_from_text, repair_and_load_json
            candidates = parse_json_from_text(raw_text) or []
            for cand in candidates:
                try:
                    parsed = repair_and_load_json(cand)
                except Exception:
                    try:
                ...
                for m in list(missing):
                    if m in parsed and m not in self.environment.execution_data:
                        val = parsed[m]
                        if isinstance(val, (dict, list)):
                            try:
                                ...
                                val = str(val)
                        self.environment.execution_data[m] = val
        except Exception:
            pass
        # Last resort: assign raw text to all still-missing outputs
        still_missing = [o for o in task_outputs if o not in self.environment.execution_data]
        if still_missing:
            from ..core.logging import logger
            logger.warning(f"Force-filling {still_missing} for task '{task.name}' from raw agent response.")
            for m in still_missing:
                self.environment.execution_data[m] = raw_text

    def _try_direct_output_extraction(self) -> Optional[str]:
        """Try to extract the final output directly from execution_data.

        If the workflow graph's end nodes have outputs that are already present
        in execution_data, return them directly.  This avoids an extra LLM call
        that can mangle answer formatting (e.g. dropping oxed{} in MATH).
        Falls back to None so the caller can use the LLM-based extraction.
        """
        try:
            end_node_names = self.graph.find_end_nodes()
            if not end_node_names:
                return None
            execution_data = self.environment.get_all_execution_data()
            # Collect the output field names of all end nodes.
            end_output_names = []
            for node_name in end_node_names:
                node = self.graph.get_node(node_name)
                for out in getattr(node, "outputs", []) or []:
                    end_output_names.append(out.name)
            if not end_output_names:
                return None
            # Check that all expected outputs are present.
            missing = [name for name in end_output_names if name not in execution_data]
            if missing:
                return None
            # Single output (most common): return the value directly.
            if len(end_output_names) == 1:
                value = execution_data[end_output_names[0]]
                return str(value) if value is not None else None
            # Multiple outputs: join them.
            parts = []
            for name in end_output_names:
                value = execution_data.get(name)
    ...
    def _validate_workflow_structure(self, inputs: dict, **kwargs):
        ...
        for node in self.graph.nodes:
            ...
            if not all(input_name in node_input_names for input_name in node_required_input_names):
                ...
                if is_initial_node:
                    # Auto-fill missing inputs from "goal" if available
                    goal_value = inputs.get("goal", "")
                    if goal_value and missing_required_inputs:
                        for missing_input in list(missing_required_inputs):
                            inputs[missing_input] = goal_value
                            logger.warning(f"Auto-filled missing input '{missing_input}' for initial node '{node.name}' from 'goal' value.")
                    else:
                        raise ValueError(
                    ...
                    logger.warning(
                        ...
                        f"Execution may fail if predecessors do not provide these outputs at runtime."
                    )
