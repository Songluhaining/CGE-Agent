...
class WorkFlowGraph(BaseModule):
    ...
    def _find_all_loops(self) -> Dict[str, list]:
        ...
        # merge same loops with different starts (鍥犱负鍚屼竴涓幆鍙兘鍦ㄤ箣鍓嶇殑閬嶅巻涓湁涓嶅悓鐨剆tart_node)
    ...
    def remove_edge(self, edge: Union[Tuple[str, str], WorkFlowEdge], update_graph: bool = True, **kwargs):
        ...
        if isinstance(edge, tuple):
            ...
            source, target = matched_edge.source, matched_edge.target
        else:
            raise TypeError("edge must be a tuple (source, target) or WorkFlowEdge instance")

        self.edges.remove(matched_edge)
        if self.graph.has_edge(source, target):
            removed = False
            for key, attrs in list(self.graph[source][target].items()):
                ref = attrs.get("ref") if isinstance(attrs, dict) else None
                if ref is matched_edge or ref == matched_edge:
            ...
            if not removed:
                ...
                if edge_keys:
                    self.graph.remove_edge(source, target, key=edge_keys[0])
        if update_graph:
            self.update_graph()

    def remove_node(self, node: Union[str, WorkFlowNode], update_graph: bool = True, **kwargs):

        node_name = node if isinstance(node, str) else node.name
        if not self.node_exists(node_name):
            raise ValueError(f"Node `{node_name}` does not exists!")

        self.edges = [edge for edge in self.edges if edge.source != node_name and edge.target != node_name]
        self.nodes = [existing for existing in self.nodes if existing.name != node_name]
        self.graph.remove_node(node_name)
        if update_graph:
            self.update_graph()

    def get_incoming_edges(self, node: Union[str, WorkFlowNode]) -> List[WorkFlowEdge]:
        node_name = node if isinstance(node, str) else node.name
        if not self.node_exists(node=node_name):
            raise ValueError(f"Node `{node_name}` does not exists!")
        return [edge for edge in self.edges if edge.target == node_name]

    def get_outgoing_edges(self, node: Union[str, WorkFlowNode]) -> List[WorkFlowEdge]:
        node_name = node if isinstance(node, str) else node.name
        if not self.node_exists(node=node_name):
            raise ValueError(f"Node `{node_name}` does not exists!")
    ...
    def get_candidate_children_nodes(self, completed_nodes: List[Union[str, WorkFlowNode]]) -> List[str]:
        ...
        if has_loop:
                ...
                if self.is_loop_end(node=node_name):
                    ...
                    for child in children_nodes:
                        if self.is_loop_start(node=child):
                            ...
                            # node_name鏄幆鐨勭粨鏉燂紝浣嗘槸瀛愯妭鐐逛笉鏄幆鐨勫紑濮嬫椂锛岄渶瑕佹鏌hild鏄惁宸茬粡completed锛屽彧娣诲姞鏈畬鎴愮殑浠诲姟
            ...
            # No loop exists: collect all descendant children and keep only unfinished ones.
