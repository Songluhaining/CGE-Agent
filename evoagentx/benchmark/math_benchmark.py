...
class AFlowMATH(MATH):
    ...
    def evaluate(self, prediction: Any, label: Any) -> dict:
        metrics = super().evaluate(prediction=prediction, label=label)
        # Map solve_rate to f1 key for optimizer compatibility
        # (LLMWorkflowOptimizer tracks the primary score under the "f1" key)
        metrics["f1"] = metrics["solve_rate"]
        metrics["em"] = metrics["solve_rate"]
        return metrics

    def _get_id(self, example: Any) -> Any:
        if "id" in example:
            return example["id"]
        return example.get("problem", "")[:80]
