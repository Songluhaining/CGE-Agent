...
class CostManager:
    ...
    @atomic_method
    def snapshot(self):
        """Return a cheap immutable snapshot for later delta accounting."""
        total_tokens, total_cost = self.compute_total_cost()
        return {
            "total_input_tokens": dict(self.total_input_tokens),
            "total_output_tokens": dict(self.total_output_tokens),
            "total_tokens": dict(self.total_tokens),
            "total_input_cost": dict(self.total_input_cost),
            "total_output_cost": dict(self.total_output_cost),
            "total_cost": dict(self.total_cost),
            "aggregate_tokens": total_tokens,
            "aggregate_cost": total_cost,
        }

    def diff(self, before: dict, after: dict = None):
        """Compute aggregate token/cost delta between two snapshots."""
        before = before or {}
        after = after or self.snapshot()
        return {
            "tokens": int(after.get("aggregate_tokens", 0) - before.get("aggregate_tokens", 0)),
            "cost": float(after.get("aggregate_cost", 0.0) - before.get("aggregate_cost", 0.0)),
        }
    ...
    def update_cost(self, cost: Cost, model: str):
        ...
        # Temporarily silence per-request cost logs during optimization runs.
        # Accounting is still accumulated in the manager for snapshots / diffs.
        # total_tokens, total_cost = self.compute_total_cost()
